...
function Catalog() {
    ...
    // Log catalogName immediately
    useEffect(() => {
        console.log("CATALOG NAME RECEIVED:", catalogName);
    }, [catalogName]);
    ...
    useEffect(() => {
        ; (async () => {
            try {
                ...
                console.log("ALL CATEGORIES FETCHED:", res);

                // Log the full category matching process
                const matchedCategories = res.filter(
                ...
                if (matchedCategories.length === 0) {
                    ...
                    console.log("Available categories:", res.map(cat => ({
                        name: cat.name,
                        transformedName: cat.name.split(" ").join("-").toLowerCase()
                    })));
                    return;
                }
